...
# 构建估算器
estimator = tf.estimator.Estimator(model_fn=my_model, model_dir='./myestimatormode', params={'learning_rate': 0.01},
                                   config=tf.estimator.RunConfig(session_config=session_config))

tensors_to_log = {"钩子函数输出": "loss","权重W：":"weight","偏置":"bias"}
logging_hook = tf.train.LoggingTensorHook(  tensors=tensors_to_log, every_n_iter=1)
# 匿名输入方式
estimator.train(lambda: train_input_fn(train_data, batch_size), steps=100,hooks=[logging_hook])
tf.logging.info("训练完成.")  # 输出训练完成
print(estimator.get_variable_value("weight"),estimator.get_variable_value("bias"))
